...
let sentence = input.sentence.trim().split(' ').filter(word => word).join(' ');
// Get Words Lengths Into Prompting
let wordsLength = input.sentence.trim().split(' ').filter(word => word).length;
// Dashboard
let message;
console.clear();
if (wordsLength) {
    ...
    message = `You haven't typed any word!`;
}
